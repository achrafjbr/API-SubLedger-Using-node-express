...
const subscribe = async (subscription) => {
    ...
    if (!subscriptionData) return new DIMessage()
...
const getUserSubscriptions = async (userId) => {
    ...
    if (!user) return new DIMessage().message(new ErrorMessage(404, "User not found"));
    const userSubscriptions = await Subscription.find({ user: userId }).populate('user', '[name, email, password]');
    if (!userSubscriptions) return new DIMessage().message(new ErrorMessage(404, "You have not no subscription"));
    return new DIMessage().message(
...
const getSubscription = async (subscriptionId) => {
    ...
    if (!subscription) return new DIMessage().message(new ErrorMessage(404, "User not found"));
    return new DIMessage().message(
...
const updateSubscription = async (subscriptionId) => {
    let subscription = await Subscription.findByIdAndUpdate(subscriptionId, {
        $set: {
            name,
            price,
            cover,
    ...
    if (!subscription) return new DIMessage().message(new ErrorMessage(404, "Subscription not found"));
    return new DIMessage().message(
        new SuccessMessage(200, 'Success', subscription)
    );
}
